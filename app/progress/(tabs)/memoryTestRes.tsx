...
import { MemoryTestData } from '@/app/scripts/filesystem/types';
import fsConstants from '@/app/scripts/filesystem/constants';

export default function MemoryTestRes() {
  const [data, setData] = useState<MemoryTestData[] | null>(null);
  ...
  useEffect(() => {
    const gettingData = () => {
      const { dirName, memoryTestFile } = fsConstants;
      const data = getData<MemoryTestData[]>(
        Paths.document,
        dirName,
        memoryTestFile
      );
